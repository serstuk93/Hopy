...
class Basic_Player(pygame.sprite.Sprite):
    """
    basic Player class handling creating trail, images, animations, and drawing to screen
    """
    ...
    def __init__(
        ...
        sprite_sheet_image = pygame.image.load(
            f"resources/heads/head_piskel{self.playerid[1:]}.png"
        ).convert_alpha()
        ...
        for i in range(run_animation_frames):
            ...
            tmp_img = pygame.transform.rotate(tmp_img, angle)
            run_animation_list.append(tmp_img)

        for i in range(
            run_animation_frames, run_animation_frames + jump_animation_frames
        ):
            tmp_img = sprite_sheet.get_image(i, 32, 32, 1, BLACK)
            tmp_img = pygame.transform.rotate(tmp_img, angle)
    ...
    def reset(self):
        """
        reset function for whole class
        """
    ...
    def move(self, offsetx, offsety):
        """
        creates new position by summing previous values with offset values which are "velocity" values
        """
    ...
    def rotation(self, angle):
        """
        modifies rotation of player image"""
        self.rot = (self.rot - angle) % 360
        # self.head_image_copy = pygame.transform.rotate(self.head_image, self.rot)
        self.head_image_copy = pygame.transform.rotate(
            self.animations[self.action][self.frame], self.rot
        )
        self.head_rect = self.head_image_copy.get_rect()

    def update_animation(self):
        """
        iterating in slides of images based on action type. Animation cooldown is time between frame change in ms.
        Function also updates rotation of Player image."""
    ...
    def draw_player(self):
        """
        Creates prediction paths with front_position_sensor function call. Draws Players image with transparent background.
        Transparency is handled by set_aplha method.For correct allign at the same place of trail width and height are
        recalculated everytime. The reason is pygame during rotation of image changes width and height of image.
        That change causes misaligment of position. Half_height and half_width are handling that.
        ...
        if self.head_image_copy is None:
            self.destinate.blit(
                self.animations[self.action][self.frame], self.front_predict_position
            )
        else:
            ...
            self.destinate.blit(
                self.head_image_copy,
                (
                    self.front_predict_position[0] - half_width,
                    self.front_predict_position[1] - half_height,
                ),
            )
    ...
    def create_trail(self):
        """
        Creating list of previous and actual position of player. If previous velocity ie speed is the same as actual then
        there is no rotation and movement is straight. To optimize drawings of trails self.c value is created and incremented.
        Only when value reaches number 5, new trail position is added. If previous and current speed is different, its becouse
        of rotation and so self.c increment can be different to correctly draw. After jump to prevent problem with missing trail,
        "if" condition is used to check jump status and value is started with 5. So no trail position is missed.
        """
    ...
    def draw_trail(self, trail_list):
        """
        Draws positions from list as simple points at desktop. Positions are corrected by minus 5 value becouse drawing surface size is 10,
        and -5 causes to center drawing trail.
        """
    ...
    def player_score(self):
        """
        score is calculated by number of items in trail list
        """
        self.score = len(self.trail)

    def get_pixel_color(self):
        """
        Function to get info about color of pixel infront of player. Color of pixel is important for checking collision in pixel_colision.
        If color is different than black( becouse black is color of background and its not used by any player) then collision occurs.
        front_position_sensor updates prediction trail paths. Surface of size 4x4 is created and its filled by white color.
        It is drawn infront of player. This point is for debugging and can be disabled without bugging the game.
        But with disabling drawing of player in hopy.py function (any_pl.draw_player()) it is visible where points for colision start is.
        """
        self.front_position_sensor()
        self.pixel_color_point = pygame.Surface((4, 4))
        self.pixel_color_point.fill((255, 255, 255, 0))
        self.rounded_pos = (
            int(self.front_predict[-1][0] - 2),
            int(self.front_predict[-1][1] - 2),
        )
    ...
    def front_position_sensor(self):
        """
        Function for getting list of values infront of player path. There are important values. First is list front_predict
        where last position for debugging in get_pixel_color are generated as colision point. front_predict_steps is value for steps infront of player.
        Next important variable is front_predict_jump where point for launching jump is calculated.
        """
        self.front_predict = []
        ...
        for i in range(0, self.front_predict_steps):
            self.front_predict_velocity = self.vel(
                self.front_predict_velocity, self.front_movement_angle
            )

            self.front_predict_position = [
                round(
                    (self.front_predict_position[0] + self.front_predict_velocity[0]), 2
                ),
                round(
                    (self.front_predict_position[1] + self.front_predict_velocity[1]), 2
                ),
            ]
        ...
        for i in range(0, 5):
            self.front_predict_jump = [
                round((self.front_predict_jump[0] + self.front_predict_velocity[0]), 2),
